...
const PredictionTab = ({ config, API, modelInfo }) => {
    ...
    const generateMockPrediction = (data) => {
        const classes = ['CONFIRMED', 'CANDIDATE', 'FALSE POSITIVE', 'NOT DISPOSITIONED'];
        ...
        const explanations = {
            'CONFIRMED': 'This KOI shows characteristics consistent with a confirmed exoplanet based on transit depth, period, and signal-to-noise ratio.',
            'CANDIDATE': 'This object displays promising exoplanet characteristics but requires further validation.',
            'FALSE POSITIVE': 'The signal is likely caused by astrophysical false positives or instrumental effects.',
            'NOT DISPOSITIONED': 'This observation requires additional analysis for classification.'
        };
        ...
        return {
            ...
            message: 'Mock prediction (ML service unavailable)',
            stored: false,
            isMock: true
        };
    };
    ...
    const handlePredict = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                const isMock = response.data.data?.is_mock || 
                              response.data.data?.prediction?.is_mock ||
                              response.data.isMock ||
                              response.data.has_mock_data;
                setResult({
                    ...response.data,
                    isMock: isMock
                });
            ...
            console.error("KOI Prediction failed, using mock data:", error);
            
            const endTime = Date.now();
            setPredictionTime(((endTime - startTime) / 1000).toFixed(2));
            
            const mockResult = generateMockPrediction(predictionData);
        ...
        config.features.forEach(feature => {
            ...
            switch (feature.name) {
                case "koi_period":
                    ...
                    break;
                case "koi_impact":
                    randomValue = (Math.random() * (1 - 0.01) + 0.01).toFixed(3);
                    break;
                case "koi_duration":
                    randomValue = (Math.random() * (20 - 1) + 1).toFixed(2);
                    break;
                case "koi_depth":
                    randomValue = (Math.random() * (5000 - 50) + 50).toFixed(1);
                    break;
                case "koi_prad":
                ...
                case "koi_teq":
                    ...
                    break;
                case "koi_insol":
                    randomValue = (Math.random() * (10000 - 1) + 1).toFixed(2);
                    break;
                case "koi_model_snr":
                    randomValue = (Math.random() * (100 - 5) + 5).toFixed(1);
                    break;
                case "koi_steff":
                    randomValue = (Math.random() * (10000 - 3000) + 3000).toFixed(0);
                    break;
                case "koi_slogg":
                    randomValue = (Math.random() * (5 - 3) + 3).toFixed(3);
                    break;
                case "koi_srad":
                    randomValue = (Math.random() * (5 - 0.5) + 0.5).toFixed(3);
                    break;
                case "koi_kepmag":
                    randomValue = (Math.random() * (20 - 10) + 10).toFixed(3);
                    break;
                default:
                ...
                <div>
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-4 animate-fade-in">
                            {(result.data?.prediction?.is_mock || result.isMock) && (
                                <div className="bg-yellow-900/50 border border-yellow-700 rounded-lg p-3">
                                    <div className="flex items-center text-yellow-300">
                                        <span className="text-lg mr-2">⚠️</span>
                                        <span className="text-sm">Using simulated data (ML service unavailable)</span>
                                    </div>
                                </div>
                            )}
                            <div className="bg-gray-900 rounded-lg p-6 border border-purple-500">
                            ...
                            {/* Charts Display */}
                            {result.diagrams?.charts && Object.keys(result.diagrams.charts).length > 0 && (
                                <div className="bg-gray-900 rounded-lg p-4 space-y-4">
                                    <h5 className="font-semibold text-white mb-4">
                                        📊 Visual Analysis & Diagrams
                                        <span className="text-sm text-purple-400 ml-2">
                                            ({Object.keys(result.diagrams.charts).length} charts)
                                        </span>
                                    </h5>
                                    <div className="space-y-6">
                                        {Object.entries(result.diagrams.charts).map(([chartName, chartData]) => (
                                                ...
                                                {chartData ? (
                                                    <>
                                                        <img 
                                                            ...
                                                            alt={chartName}
                                                            className="max-w-full h-auto rounded-lg border border-purple-600 shadow-lg hover:shadow-xl transition-shadow duration-300"
                                                            onError={(e) => {
                                                                console.log(`Chart ${chartName} failed to load`);
                                                                e.target.style.display = 'none';
                                                            }}
                                                        />
                                                        <p className="text-gray-400 text-sm mt-3 capitalize font-medium">
                                                            {chartName.replace(/([_-])/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                                                        </p>
                                                    </>
                                                ) : (
...
// Enhanced Bulk Analysis Tab for KOI
